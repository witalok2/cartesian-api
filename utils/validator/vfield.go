...
import (
	...
	time "github.com/cartesian-api/utils/time"
)
...
func (v vField) toTimeEUA() time.TimeEUA {
	value, ok := v.Interface.(time.TimeEUA)
	if !ok {
...
func (v vField) toTimestamp() time.Timestamp {
	value, ok := v.Interface.(time.Timestamp)
	if !ok {
...
func (v vField) isTimeEUA() bool {
	_, ok := v.Interface.(time.TimeEUA)
	return ok
}

func (v vField) isTimestamp() bool {
	_, ok := v.Interface.(time.Timestamp)
	return ok
}
